...
/*    source_odom_sub_ = nh_priv_.subscribe(params.gt_topic_name+params.source_odom_name, 10, &Node::GTSourceCallback, this);
    ...
    output_file_.open("/home/marklee/data/jpdaf_noise_data.csv");
    output_file_ << "Mx,y,x,y,x,y,x,y,Tx,y,x,y,x,y" << endl;
...
void Node::detectionCallback(const geometry_msgs::PoseArray& in_PoseArray)
{
    ...
    for (int i =0; i < (int)in_PoseArray.poses.size() ; i++)
    {
        geometry_msgs::Pose value_Pose;
        value_Pose.position.x = in_PoseArray.poses[i].position.x;
        value_Pose.position.y = in_PoseArray.poses[i].position.y;
        noise_PoseArray.poses.push_back(value_Pose);
...
ROS_ERROR("total measurement size: %lu ", noise_PoseArray.poses.size() );
    ...
    bounding_boxes_msgs_buffer_.push_back(noise_PoseArray);
    track(true);
}
...
//    darknet_ros_msgs::BoundingBoxes latest_detection;
    ...
    if(called_from_detection)
    {
        latest_pose_detection = bounding_boxes_msgs_buffer_.back();
    }
    else
    {
        std::vector<geometry_msgs::Pose> emptyVector;
        latest_pose_detection.poses = emptyVector;
    }

//    auto detections = get_detections(latest_detection);
    auto detections = get_detections(latest_pose_detection);


    ROS_INFO("Detections:");
    for(int d=0; d<(int)detections.size(); d++)
    {
        cout << (int)(detections[d].getVect())(0) << "," <<  (int)(detections[d].getVect())(1) << ",";
        output_file_ <<  (int)(detections[d].getVect())(0) << "," <<  (int)(detections[d].getVect())(1) << "," ;
    }
    cout << endl;

//    output_file_ << "1,2,3,4,5,6" << endl;
//    output_file_ <<  (int)(detections[0].getVect())(0) << "," <<  (int)(detections[0].getVect())(1) << "," <<
//    (int)(detections[1].getVect())(0) << "," <<  (int)(detections[1].getVect())(1) << "," <<
//    (int)(detections[2].getVect())(0) << "," <<  (int)(detections[2].getVect())(1) << "," << endl;
...
//            time_step = ros::Time::now().toSec() - last_timestamp_from_rostime;
        ...
        if(time_step != 0.0)//Sometimes Darknet ros publishes consecutive detections (bounding boxes and detections) with same timestamp, this happens when no new image is available in the yolo's buffer of images to perform detection
        {
            ...
            //PREDICTION
            ROS_WARN("=========== predict start =========");
            std::vector<Eigen::Vector2f> projected_predictions;

//            cout << "track size: " << tracks_.size() << endl;
...
//            cout << "x predict states: " << projected_predictions[0] << endl;
...
std::vector<Detection> Node::get_detections(const geometry_msgs::PoseArray latest_detection)
{
    std::vector<Detection> norm_det;
